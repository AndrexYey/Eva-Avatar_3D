#!/bin/bash
# E.V.A — lanzador para macOS (doble clic en Iniciar-EVA-MAC.command)
set -u
cd "$(dirname "$0")"
...
if [ ! -x bin/bun ]; then
  ...
  ARCH=$(uname -m)
  if [ "$ARCH" = "arm64" ]; then B=bun-darwin-aarch64; else B=bun-darwin-x64; fi
  mkdir -p bin
...
# ── 2. Python 3 (macOS lo pide con instalador gráfico si falta) ────────
if ! command -v python3 >/dev/null 2>&1; then
  echo "[2/4] Python no encontrado. Se abrirá el instalador de Apple; instálalo y vuelve a ejecutar este archivo."
...
if [ ! -x .venv/bin/python ]; then python3 -m venv .venv || { echo "  ERROR al crear el entorno local."; read -r -p "Enter para salir"; exit 1; }; fi
